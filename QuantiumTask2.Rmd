# Quantium Virtual Internship - Exeprimentation and Uplift testing - Task 2
...
*Importing Datasets*
```{r}
sdata <- fread(paste0("QVI_data2.csv"))
...
sdata[, YEARMONTH := year(DATE) * 100 + month(DATE)]

#Next, we define the measure calculations to use during the analysis.
measureOverTime <- sdata[, .(totSales = sum(TOT_SALES),
...
#Using the functions for calculating magnitude
...
Now scaling the control store's sales to a level similar to control for any differences between the two stores outside of the trial period.
...
#Since there are 8 months in the pre-trial period, hence degrees of freedom are 7
degreesOfFreedom <- 7
...
months.
Assesing the same for the number of customers
...
ggplot (trialAssessment, aes (TransactionMonth, nCusts, color = Store_type)) + 
  ...
  geom_line () + labs (x = "Month of operation",
...
##For Trial Store 86
```{r}
measureOverTime <- as.data.table(sdata %>% group_by(STORE_NBR, YEARMONTH) %>%
                                   mutate(totSales = sum(TOT_SALES),
                                          nCustomers = uniqueN(LYLTY_CARD_NBR),
                                          nTxnPerCust = uniqueN(TXN_ID)/uniqueN(LYLTY_CARD_NBR),
                                          nChipsPerTxn = sum(PROD_QTY)/uniqueN(LYLTY_CARD_NBR),
                                          avgPricePerUnit = sum(TOT_SALES)/sum(PROD_QTY) ) %>%
                                   select(STORE_NBR, YEARMONTH, totSales, nCustomers, nTxnPerCust,
                                          nChipsPerTxn, avgPricePerUnit) %>%
                                   distinct() %>% arrange(STORE_NBR, YEARMONTH))
...
score_nSales <- merge(corr_nSales,
                      magnitude_nSales,
                      by = c ("Store1",
                              "Store2"))[, scoreNSales := corr_measure*corr_weight +
                                           mag_measure*(1 - corr_weight)]
...
control_store <- score_Control[Store1 == trial_store,][ order(- finalControlScore)][2, Store2]
control_store
# control_store <- score_Control %>% filter(Store1 == trial_store) %>% arrange(desc(finalControlScore))
# control_store[2, Store2]
```
...
Great, the trend in number of customers is also similar.
...
#Since, there are 8 months in the pre-trial period, hence, degrees of freedom
degreesOfFreedom <- 7
